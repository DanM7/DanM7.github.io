const DEBUG_TOGGLE_GLOBAL = true;
...
// heroAnimationDimensions[ANIMATION_HERO_IDLE] =       { "w": 36, "h": 60, "offsetX": 32, "offsetY": 13 };
// heroAnimationDimensions[ANIMATION_HERO_RUN] =        { "w": 40, "h": 60, "offsetX": 42, "offsetY": 13 };
// heroAnimationDimensions[ANIMATION_HERO_JUMP] =       { "w": 36, "h": 52, "offsetX": 32, "offsetY":  8 };
// heroAnimationDimensions[ANIMATION_HERO_JUMP_EXTRA] = { "w": 36, "h": 30, "offsetX": 32, "offsetY": 15 };
// heroAnimationDimensions[ANIMATION_HERO_FALL] =       { "w": 36, "h": 60, "offsetX": 32, "offsetY": 13 };

// ToDo: if falling, you should be able to extra jump.
...
Hero.prototype.canExtraJump = function () {
    return (this.alive && !this.isFrozen && 
...
Hero.prototype.canSingleJump = function () {
    return (this.alive && !this.isFrozen && 
        (this.touchingDownCount > 0 || this.isLedgeGrabbing)
    );
}
...
Hero.prototype._getAnimationName = function () {
    ...
    else if (this.body.velocity.x !== 0 && this.touchingDownCount > 0 && !this.isSliding) {
...
PlayState._handleCollisions = function () {
    ...
    this.landscape.forEach(function (land) {
        this.game.physics.arcade.collide(this.hero, land
            , this._onHeroCollisionWithLand, null, null
        );
    }, this);
    
    // this.game.physics.arcade.overlap(this.hero, this.landscape,
    //     this._onHeroInsideLandscape, null, this);
...
PlayState._handleInput = function () {
    let xDir = 0;
    if (this.keys.left.isDown && !this.hero.body.touching.left) { // move hero left //  && this.hero.body.overlapX == 0
        xDir -= 1;
    }
    if (this.keys.right.isDown && !this.hero.body.touching.right) { // move hero right //  && this.hero.body.overlapX == 0
        xDir += 1;        
    }
...
PlayState._onHeroCollisionWithLand = function (hero, land) {
    ...
    let touchingUp = (hero.body.touching.up),
        touchingDown = false,
        touchingLeft = (hero.body.touching.left),
        touchingRight = (hero.body.touching.right);

    if (hero.body.touching.down === true) {
        if ((hero.body.x + hero.body.width) === land.x) {
            touchingDown = false;
        }
        // else if (hero.body.overlapX > 0) {
        //     touchingDown = false;
        //     hero.body.x -= hero.body.overlapX;
        //     hero.body.overlapX = 0;
        // }
        else {
    ...
    if (hero.canLedgeGrab === true && land.grab === true) {
        if (touchingRight || touchingLeft) {
            if (!touchingUp && !touchingDown) {
                // Wall-Slide or Ledge-Grab?
...
PlayState._loadLevel = function (data) {
    ...
    this.landscape.enableBody = true;
    this.landscape.physicsBodyType = Phaser.Physics.ARCADE;
    ...
    data.landscape.forEach(function (land) {
        ...
        var landX = (land.x32) ? land.x32*32 : land.x;
        var landY = (land.y32) ? land.y32*32 : land.y;
        ...
        for (var rX = 0; rX <= rXTotal; rX++) {
            for (var rY = 0; rY <= rYTotal; rY++) {
                var landItem = this.game.add.sprite(landX + (rX*landWidth), landY + (rY*landHeight), 'landscape', landFrame);
                this.game.physics.enable(landItem);
                landItem.body.allowGravity = false;
                landItem.body.immovable = true;

                // custom properties:
                landItem.grab = land.grab;

                this.landscape.add(landItem);
            }
        }
    }, this);
