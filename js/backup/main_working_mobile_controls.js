...
PlayState._handleTouchDPadInput = function () {
    PlayState.keys.up.isDown = false;
    PlayState.keys.down.isDown = false;
    PlayState.keys.left.isDown = false;
    PlayState.keys.right.isDown = false;
    if (this.game.input.pointer1.isDown) {
        let p1X = this.game.input.pointer1.x;
        let p1Y = this.game.input.pointer1.y;

        //circumferencePoint could be useful?

        let dPadDegrees = 0;
        if (dpadCircleArea.contains(p1X, p1Y)) {
            ...
            // ToDo: don't count up or down if we're real close to the 
            // X Axis. It makes sliding between left and right and barely
            // going into the "up zone" and causing a jump feel weird.
            ...
            */
            ...
            if (45 <= dPadDegrees && dPadDegrees < 135) {
                // Up:
                //this.coinPickupCount = 2;
                PlayState.keys.up.isDown = true;
                if (keyUpDurationDown === null) {
                    keyUpDurationDown = new Date();
                }
            }
            else if (135 <= dPadDegrees && dPadDegrees < 225) {
                // Left:
            ...
            else if (225 <= dPadDegrees && dPadDegrees < 315) {
                // Down:
                //this.coinPickupCount = 4;
                PlayState.keys.down.isDown = true;
                if (keyUpDurationDown != null) {
                    keyUpDurationDown = null;
                }
...
PlayState._handleInput = function () {
    ...
    PlayState._handleTouchDPadInput();
...
PlayState._createHud = function () {
    ...
    buttonDown = this.game.add.sprite(
        buttonUpX, buttonUpY + controlHeightUD + 2, 'controlsDown'
    );
    ...
    buttonRight = this.game.add.sprite(
        buttonLeft.x + controlWidthLR - 3, buttonLeft.y, 'controlsRight'
    );
    ...
    // controlsDPadCircle.events.onInputDown.add(controlsTouchDPadDown, this);
    // controlsDPadCircle.events.onInputUp.add(controlsTouchDPadUp, this);
    // controlsDPadCircle.events.onInputOver.add(controlsTouchDPadOver, this);
    // controlsDPadCircle.events.onInputOut.add(controlsTouchDPadOut, this);
    
    // controlsDPad = this.game.add.sprite(
    //     buttonUpX - 33, buttonUpY + 15, 'controlsDPad'
    // );
    // controlsDPad.alpha = 0.7;
    ...
    controlButtons.forEach(function(controlButton) {
        ...
        // controlButton.events.onInputDown.add(controlsButtonPress, this);
        // //controlButton.events.onInputOver.add(controlsButtonPress, this);
        // controlButton.events.onInputUp.add(controlsButtonRelease, this);
        // controlButton.events.onInputOut.add(controlsButtonRelease, this);
        PlayState.hud.add(controlButton);
    });

    this.hud.add(controlsDPadCircle);
    //this.hud.add(controlsDPad);
