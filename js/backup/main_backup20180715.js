...
function Hero(game, x, y) {
    ...
    // player settings:
    this.jumpSpeed = 400;
    this.isJumpingSingle = false; // start the game not jumping.
    this.isJumpingExtra = false;
    this.extraJumpsCurrent = 0;
    this.extraJumpsMax = 1;

    // animations
    this.animations.add('stop', [0, 0, 0, 0, 0, 1, 2, 3, 0, 0, 0, 0], 4, true);
    this.animations.add('crouch', [4, 4, 4, 4, 4, 5, 6, 7, 4, 4, 4, 4], 4, true);
    this.animations.add('run', [8, 9, 10, 11, 12, 13], 8, true); // 8fps looped

     // ToDo: loop last few frames of these 2 and split them so they're the initial action then the loop.
    this.animations.add('jump', [14, 15, 16, 17], 12, false);
    this.animations.add('jumpExtra', [18, 19, 20, 21], 12, true);
    this.animations.add('fall', [17, 22, 23], 8, false);
...
Hero.prototype.uncrouch = function () {
...
Hero.prototype.canSingleJump = function () {
    return (this.alive && !this.isFrozen && this.body.touching.down);
}
...
LoadingState.preload = function () {
    ...
    this.game.load.spritesheet('hero', 'images/bowen/adventurer-1.3-Sheet.png', 100, 73, 95);
...
PlayState._handleInput = function () {
    ...
    if (this.keys.right.isDown) { // move hero right
        xDir += 1;        
    }
    ...
    if (this.hero.body.velocity.y === 0 && this.keys.down.isDown) {
        this.hero.crouch();
    }
    else {
        this.hero.uncrouch();
